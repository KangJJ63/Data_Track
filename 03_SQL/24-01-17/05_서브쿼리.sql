...
select e.emp_id
from (select * from emp where dept_id = 100)  e;
...
select dept_id, dept_name, concat('$', sal) "평균급여"
from (
    select  d.dept_id, d.dept_name , 
			round(avg(e.salary), 2) "sal"
...
select dept_id, concat('$ ', sal) as "평균급여"
from (
...
-- TODO: 직원의 ID(emp.emp_id)가 145인 직원보다 많은 연봉을 받는 
-- 직원들의 이름(emp.emp_name)과 급여(emp.salary) 조회.급여가 큰 순서대로 조회
select emp_name, salary
from   emp
where  salary > (select salary from emp where emp_id = 145);
select salary from emp where emp_id = 145;

-- TODO: 직원의 ID(emp.emp_id)가 150인 직원과 업무(emp.job_id)와
--  상사(emp.mgr_id)가 같은 직원들의 
...
--  직원들의 id(emp.emp_id), 이름(emp.emp_name), 급여(emp.salary)를 조회.
select  emp_id, emp_name, salary
from    emp
where   salary > (select max(salary) from emp where emp_name = 'John');


-- TODO: 급여(emp.salary)가장 많이 받는 직원이 속한 부서의 이름(dept.dept_name), 
--     위치(dept.loc)를 조회.
select  dept_id, dept_name, loc
from    dept
where   dept_id = (select dept_id from emp where salary = 
...
-- TODO: 급여(emp.salary)를 제일 많이 받는 직원들의 이름(emp.emp_name), 
-- 부서명(dept.dept_name), 급여(emp.salary) 조회. 
...
-- TODO: 전체 직원들 중 담당 업무 ID(emp.job_id) 가 'ST_CLERK'인 직원들의 평균 급여보다
...
and   ifnull(job_id, 'abc') != 'ST_CLERK';
...
-- TODO: EMP 테이블에서 업무(emp.job_id)가 'IT_PROG' 인 직원들의 
-- 평균 급여보다 더 많은 급여를 받는 직원들의 id(emp.emp_id), 이름(emp.emp_name),
--  급여(emp.salary)를 급여 내림차순으로 조회.
select emp_id, emp_name, salary
from   emp 
...
-- 부하 직원들의 ID(emp_id), 이름(emp_name), 업무(job_id), 입사년도(hire_date-년도만출력), 
-- 급여(salary)를 조회

select  emp_id, emp_name,mgr_id, job_id, year(hire_date) "입사년도", salary
from    emp
where   mgr_id in (select emp_id from emp where emp_name = 'Alexander');

select emp_id from emp where emp_name = 'Alexander';

-- 직원 ID(emp.emp_id)가 101, 102, 103 인 직원들 보다 
--  급여(emp.salary)를 많이 받는 직원의 모든 정보를 조회.
select * from emp
...
-- TODO: 전체 직원들 중 부서_ID(emp.dept_id)가 20인 부서의 
-- 모든 직원들 보다 급여(emp.salary)를 많이 받는 직원들의 정보를 sub query를 이용해 조회.
select * from emp
