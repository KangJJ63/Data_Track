...
-- emp 테이블과 dept 테이블 합친다. 
-- emp.dept_id값과 dept.dept_id가 같은 행끼리 합쳐라. (한행으로 합쳐라)
select  e.emp_id, e.emp_name, e.hire_date, d.dept_name
from    emp e inner join  dept d on  e.dept_id = d.dept_id;
-- from emp e  -> emp 테이블이름을 e로 하겠다. -> 테이블이름의 별칭
...
from   emp e join salary_grade s on salary between s.low_sal and s.high_sal;
...
-- TODO: 직원 ID 가 200 인 직원의 직원_ID(emp.emp_id), 
--  이름(emp.emp_name), 급여(emp.salary), 담당업무명(job.job_title), 
--  소속부서이름(dept.dept_name)을 조회              
select e.emp_name, e.salary, j.job_title, d.dept_name
from   emp e join dept d on e.dept_id = d.dept_id
...
-- TODO 부서별 급여(salary)의 평균을 조회. 부서이름(dept.dept_name)과 급여평균을 출력.
--     급여 평균이 높은 순서로 정렬. 
...
-- from emp e right outer join dept d  on xxx
...
-- count(*) 행수를 조회->emp정보가 없어도 1이 나옴.
select  d.dept_id, d.dept_name, count(emp_id) "직원수" 
from    emp e right join dept d on e.dept_id = d.dept_id
group by d.dept_id, d.dept_name
order by 3 desc;

-- TODO: EMP 테이블에서 부서_ID(emp.dept_id)가 90 인 
-- 모든 직원들의 id(emp.emp_id), 이름(emp.emp_name), 상사이름(emp.emp_name), 
...
-- TODO 2003년~2005년 사이에 입사한 모든 직원의 id(emp.emp_id), 이름(emp.emp_name), 
 -- 업무명(job.job_title), 급여(emp.salary), 입사일(emp.hire_date),
-- 상사이름(emp.emp_name), 상사의입사일(emp.hire_date), 소속부서이름(dept.dept_name),
--  부서위치(dept.loc)를 조회.

select e.emp_id, e.emp_name, e.hire_date, j.job_title, e.salary,
