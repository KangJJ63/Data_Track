...
SELECT 10 / 4, 10 div 4 -- div : 몫
	, 10 % 3, 10 mod 3 -- mod : 나머지
;
-- 문자열 합치기 
SELECT CONCAT("TEST",":",30) "문자열 CONCAT";
...
FROM emp
WHERE 1=1
AND comm_pct BETWEEN 0.2 AND 0.3;
...
FROM emp
WHERE 1=1
AND emp_name LIKE "%)%%" escape ')';  --  escape를 직접 지정.

SELECT emp_id
		,emp_name
FROM emp
WHERE 1=1
AND emp_name LIKE "%\%%"; -- 기본 escape 사용
...
FROM emp
WHERE 1=1
AND (job = "FI_ACCOUNT" OR salary >= 8000 )
;
...
SELECT 
    ...
    ,salary
FROM emp
ORDER BY 3,4 DESC, 2 DESC;  -- select절에 선언된 컬럼의 순번 사용(1번부터 시작) 
;
